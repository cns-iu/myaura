...
def compute_instagram_comentions_samepost(drop_set, save_path):
    #
    # Init
    #
    ...
    rCSVfile = os.path.abspath(os.path.join(os.path.dirname(__file__), os.pardir, 'tmp-data/01-instagram-epilepsy-mentions-{dicttimestamp:s}.csv.gz'.format(dicttimestamp=dicttimestamp)))
    ...
    for idx, row in df.iterrows():
        ...
        # row['matches'] is a list of dicts, each dict is a mention
        # copy row['matches'] to a new list, remove any mention whose id is in drop_set
        new_matches = [m for m in matches if m['id'] not in drop_set]
        unique_id_parent = set([m['id_parent'] for m in new_matches])
        mention_count.update(unique_id_parent)

        unique_parent_rich = [matchparent(m['id_parent'], m['parent'], m['type']) for m in new_matches]
    ...
    # Export
    wCSVfile = '{save_path:s}/02-instagram-epilepsy-comentions-{dicttimestamp:s}-samepost.csv.gz'.format(save_path=save_path, dicttimestamp=dicttimestamp)
    utils.ensurePathExists(wCSVfile)
    dfR.to_csv(wCSVfile)


    mention_count_CSVfile = '{save_path:s}/02-instagram-epilepsy-mentions-counts-{dicttimestamp:s}-samepost.csv.gz'.format(save_path=save_path, dicttimestamp=dicttimestamp)
