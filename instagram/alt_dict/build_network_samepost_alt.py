...
def build_instagram_network_samepost(save_path, metric_backbone=True):
    # Init
    ...
    #
    rCSVfile = '{save_path:s}/02-instagram-epilepsy-comentions-{dicttimestamp:s}-samepost.csv.gz'.format(save_path=save_path, dicttimestamp=dicttimestamp)
    ...
    network_name = 'Network (Instagram : Epilepsy : {dicttimestamp:s})'.format(dicttimestamp=dicttimestamp)
    ...
    print('--- Exporting ---')
    # Graph gormat
    wGfile = '{save_path:s}/04-instagram-epilepsy-network-{dicttimestamp:s}-samepost'.format(save_path=save_path, dicttimestamp=dicttimestamp)
    print('> gpickle')
    ...
    if metric_backbone:
        ...
        cols = ['source', 'target', 'count', 'proximity', 'distance', 'is_original']
    dfE = dfE[cols]
