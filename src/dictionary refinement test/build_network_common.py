...
def load_comention(comention_table, network_name=""):
...
def load_comention_csv(rCSVfile, network_name=""):
    ...
    for u, shortest_path_lenghts in all_pairs_dijkstra_path_length(G, weight='distance', disjunction=sum):
        ...
        for v, shortest_path_legth in shortest_path_lenghts.items():
            ...
            if not G.has_edge(u, v):
                ...
                G[u][v]['is_metric'] = True if (
                        (shortest_path_legth == G[u][v]['distance']) and (shortest_path_legth != np.inf)) else False
        #
        i += 1
    print('> Done.')
